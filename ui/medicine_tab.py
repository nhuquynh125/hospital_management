...
class MedicineTab(QWidget):
    ...
    def _new_prescription(self):
        # Prescriptions must be linked to a medical_record_id (created during examination).
        # Opening a blank PrescriptionDialog here would produce an orphaned prescription
        # with medical_record_id=None and doctor_id=None, which bypasses billing validation.
        QMessageBox.information(
            self, "Thông báo",
            "Đơn thuốc phải được tạo trong quá trình khám bệnh.\n"
            "Vui lòng mở hồ sơ khám của bệnh nhân (tab Lịch hẹn → Khám bệnh) "
            "và kê đơn từ đó."
        )
